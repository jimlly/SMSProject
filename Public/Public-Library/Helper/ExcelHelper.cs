...
//
//作  者：Clrs
//日　期：2013/04/12 10:12:24
//描　述：
//版　本：1.00
//修改历史记录
//版　本　　　　　修改时间　　　　　　修改人　　　　　变更内容
//1.00       2013/04/12 10:12:24       Clrs             新增
//********************************************************************************************************
#endregion
namespace Public.Helper
{
    ...
    public class ExcelHelper
    {
        ...
        /// </summary> 
        /// <param name="Path">文件名称</param> 
        /// <param name="SourceSheet">指定要读取的Excel页</param>
        /// <returns>返回一个数据集</returns> 
        public DataSet ExcelToDS(string Path,string SourceSheet)
        {
            string strConn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + Path + ";Extended Properties=\"Excel 8.0;HDR=Yes;IMEX=1\";";
            System.Data.OleDb.OleDbConnection conn = new System.Data.OleDb.OleDbConnection(strConn);
            try
            {
                ...
                System.Data.OleDb.OleDbDataAdapter myCommand = null;
                DataSet ds = null;
                strExcel = "select * from [" + SourceSheet + "$]";
                myCommand = new System.Data.OleDb.OleDbDataAdapter(strExcel, strConn);
                ds = new DataSet();
                myCommand.Fill(ds, "table1");
        ...
        /// <param name="Path">文件名称</param> 
        /// <param name="SourceSheet">指定要读取的Excel页</param>
        /// <param name="SourceColnums">Excel页中的列名称，多个以,分割</param>
        /// <returns>返回一个数据集</returns> 
        public DataSet ExcelToDS(string Path, string SourceSheet,string SourceColnums)
        {
            string strConn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + Path + ";Extended Properties=\"Excel 8.0;HDR=Yes;IMEX=1\";";
            System.Data.OleDb.OleDbConnection conn = new System.Data.OleDb.OleDbConnection(strConn);
            try
            {
                ...
                System.Data.OleDb.OleDbDataAdapter myCommand = null;
                DataSet ds = null;
                strExcel = "select " + SourceColnums + " from [" + SourceSheet + "$]";
                myCommand = new System.Data.OleDb.OleDbDataAdapter(strExcel, strConn);
                ds = new DataSet();
                myCommand.Fill(ds, "table1");
