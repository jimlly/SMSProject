...
namespace Public.Helper
{
    ...
    public class MailHelper
    {
       public enum MailServerType
        ...
        /// <param name="email">email地址</param>
        /// <returns></returns>
        public static string GetEMailLoginUrl(string email,MailServerType mailServerType)
        {
            if ((email == string.Empty) || (email.IndexOf("@") <= 0))
            {
                return string.Empty;
            ...
            int index = email.IndexOf("@");
            email = "http://"+(mailServerType==MailServerType.Smtp?"Smtp.":"Mail.") + email.Substring(index + 1);
            return email;
        }
        /// <summary>
        /// 发送邮件
