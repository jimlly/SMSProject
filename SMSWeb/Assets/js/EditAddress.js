...
function SaveContact(contactId) {
    ...
    $("select[id^='selWay']").each(function () {
        ...
        var contactWayType = $(this).attr("value");
    ...
    if (contactId != 0) {
        //修改联系人
        result = window.Meetingtel.Web.Common.AjaxHandler.SetContact(contact);
    } else {
        result = window.Meetingtel.Web.Common.AjaxHandler.AddContact(contact);
    }
    if (result) {
