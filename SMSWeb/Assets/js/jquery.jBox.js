...
(function ($) {
    $.jBox = function (content, options) {
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            $.each(stateOptions.buttons, function (k, v) {
                states.push('<button class="btn btn-small" style="margin-right:10px" value="' + v + '" style="' + ieButtonFix + '">' + k + '</button>');
            });
            states.push('</div></div>');
        });
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div class="alert no-bottom alert-' + options.icon + '">');
